...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_getSData_button = wx.Button(self, label="Get Mcstructure DATA")
        ...
        self._run_text.SetValue("Mcstructure in this world")
        self._run_setSData_button = wx.Button(self, label="Save Mcstructure  \n Data to world ")
    ...
    def _run_export(self, _):  # b'structuretemplate_mystructure:test'
        ...
        # back to bytes
        data2 = nbtf.save_to(compressed=False, little_endian=True)
        with wx.FileDialog(self, "Open NBT file", wildcard="NBT files (*.mcstructure)|*.mcstructure",
    ...
    def _run_get_slist(self):
        world = self.world.level_wrapper._level_manager._db.keys()
        currentStructure = []
        currentStructure.append("Mcstructure In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
...
export = dict(name="Edit/Export/Import Mcstructure", operation=SetBlock)
