...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_getSData_button = wx.Button(self, label="Get VILLAGE DATA")
        ...
        self._run_text.SetValue("villages in this world")
        self._run_setSData_button = wx.Button(self, label="Save village data  \n Data to world ")
    ...
    def _run_set_sdata(self, _):
        if 'VILLAGE_' not in self._run_text.GetValue():
            fixKey = "VILLAGE_" + self._run_text.GetValue()
    ...
    def _run_import(self, _):
       ...
       # nbtf = NBTFile(nbt)
        ...
        self._mode_description.SetValue(str(nbtf.to_snbt(4)))
    ...
    def _run_get_slist(self):
        world = self.world.level_wrapper._level_manager._db.keys()
        currentStructure = []
        currentStructure.append("villages data In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
                    if b'VILLAGE_' in w:
...
export = dict(name="Edit/Export/Import Villages-Data", operation=SetBlock)
