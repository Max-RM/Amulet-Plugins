...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_text.SetValue("Map in this world")
        self._run_setSData_button = wx.Button(self, label="Save Map  \n Data to world ")
    ...
    def _run_get_slist(self):
        ...
        currentStructure = []
        currentStructure.append("Map In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
                    if b'map_' in w:
...
export = dict(name="v001 Edit/Export/Import Map Updated", operation=SetBlock)
