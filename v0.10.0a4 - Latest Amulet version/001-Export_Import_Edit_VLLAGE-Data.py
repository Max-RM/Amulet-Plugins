...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_getSData_button = wx.Button(self, label="Get villages DATA")
        ...
        self._run_setSData_button = wx.Button(self, label="Save villages  \n Data to world ")
    ...
    def _run_get_slist(self):
        ...
        currentStructure = []
        currentStructure.append("villages In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
                    if b'VILLAGE_' in w:
...
export = dict(name="v001 Edit/Export/Import Villages-Data Updated", operation=SetBlock)
