...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_getSData_button = wx.Button(self, label="Get map DATA")
        ...
        self._run_text.SetValue("maps in this world")
        self._run_setSData_button = wx.Button(self, label="Save map data  \n Data to world ")
    ...
    def _run_set_sdata(self, _):
        if 'map_' not in self._run_text.GetValue():
            fixKey = "map_" + self._run_text.GetValue()
    ...
    def _run_get_slist(self):
        world = self.world.level_wrapper._level_manager._db.keys()
        currentStructure = []
        currentStructure.append("maps data In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
                    if b'map_' in w:
...
export = dict(name="Edit/Export/Import Map", operation=SetBlock)
