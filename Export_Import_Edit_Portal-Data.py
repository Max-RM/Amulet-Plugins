...
class SetBlock(wx.Panel, DefaultOperationUI):
    ...
    def __init__(
        ...
        self._run_setSData_button = wx.Button(self, label="Save portals data  \n Data to world ")
    ...
    def _run_get_slist(self):
        world = self.world.level_wrapper._level_manager._db.keys()
        currentStructure = []
        currentStructure.append("Portals data In this World")
        for w in world:
            if b'\xff' not in w:
                if b'\x00' not in w:
                    if b'portals' in w:
...
export = dict(name="Edit/Export/Import Portal-Data", operation=SetBlock)
